...
        apiKey: "AIzaSyCNFkoa4Ark8R2uzhX95NlV8Buwg2GHhvo",import { initializeApp } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-app.js";
import { getAuth, signInAnonymously, onAuthStateChanged } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-auth.js";
import { getFirestore, doc, onSnapshot, updateDoc, getDoc, setDoc } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-firestore.js";
...
    const shareFacebookBtn = document.getElementById('share-facebook-btn'); // Corrigido o ID aqui, era 'share-facebook-f' antes
...
    let isTestMode = false;
    // Removendo raffleType e totalNumbersInRaffle como variáveis de estado globais fixas
    // Elas serão lidas do numbersData do Firestore
...
            setupWhatsAppButton();
            // AQUI: A contagem de vendidos e progresso precisa ser adaptada ao tipo de rifa
...
            updateRaffleProgress(soldCount, totalNumbersInRaffle); // Passa o total de números
            updateRecentBuyers(numbersData, raffleType); // Passa o tipo para formatação correta

            if (numbersData.winner) {
                displayPublicWinner(numbersData.winner, raffleType); // Passa o tipo para formatação correta
                if (progressSection) progressSection.classList.add('hidden');
            } else {
                if (progressSection) progressSection.classList.remove('hidden');
            }
            renderNumberGrid(totalNumbersInRaffle, raffleType); // Passa o total e o tipo
            if (loadingSection) loadingSection.classList.add('hidden');
...
    function renderNumberGrid(maxNumbers, raffleType) { // Recebe maxNumbers e raffleType
...
            const numberStr = formatNumberForRaffleType(i, raffleType); // Formata o número
...
    }
    ...
    // Função auxiliar para formatar números com zeros à esquerda (relevante para as funções abaixo)
    ...
    // Função auxiliar para obter o comprimento esperado do número (relevante para as funções abaixo)
...
    function displayPublicWinner(winnerData, type) { // type adicionado aqui para formatar o número do ganhador
...
        const raffleId = rifaDocRef.id;
        const raffleType = numbersData.type || 'dezena'; // Obtém o tipo da rifa do Firestore para a formatação
        if (selectedNumbers.length === 0) return;
        
        // Formata os números antes de enviar
...
                localStorage.setItem('pendingNumbers', JSON.stringify(formattedSelectedNumbers)); // Salva números formatados
...
        const raffleId = rifaDocRef.id;
        const raffleType = numbersData.type || 'dezena'; // Obtém o tipo da rifa do Firestore
        const formattedSelectedNumbers = selectedNumbers.map(n => formatNumberForRaffleType(parseInt(n), raffleType));
        if (formattedSelectedNumbers.length === 0) return alert("Nenhum número selecionado.");
        if (!window.confirm(`-- MODO DE TESTE --\nConfirma a reserva dos números: ${formattedSelectedNumbers.join(', ')}?`)) return;
...
    function updateRecentBuyers(data, type) { // type adicionado aqui para formatar os números dos compradores
        if (!recentBuyersList) return;
        const purchases = {};
        const expectedLength = getExpectedLengthForRaffleType(type); // Usa a função auxiliar
        for (const key in data) {
            if (!isNaN(key) && key.length === expectedLength) { // Verifica o comprimento da chave
...
        
        try {
            // Obtém o tipo da rifa do numbersData para enviar à função Netlify
            const raffleTypeForLuckyNumbers = numbersData.type || 'dezena';
            const functionUrl = `/.netlify/functions/getLuckyNumbers`;
            const response = await fetch(functionUrl, { 
                method: "POST", 
                body: JSON.stringify({ theme: theme, raffleType: raffleTypeForLuckyNumbers }) // Envia o tipo
            });
            
...
                    const formattedSuggestedNumber = formatNumberForRaffleType(parseInt(s.numero), raffleTypeForLuckyNumbers); // Formata
                    
...
    }
    
    // Removendo funções de escolha aleatória e busca de número, pois não existiam na versão original
...
    if (getLuckyNumbersBtn) getLuckyNumbersBtn.addEventListener('click', getLuckyNumbers);
    ...
    // Removendo event listeners para as seções que não existem mais
...
    // if (searchNumberInput) searchNumberInput.addEventListener('keyup', (e) => { if (e.key === 'Enter') { checkSpecificNumber(); } });
    
    setupAuthListener();
    setupShareButtons();
});
