...
    // <<< NOVO: Constante para o limite de exibição de números no grid
...
    const shareFacebookBtn = document.getElementById('share-facebook-f'); // Corrigido, id era 'facebook-f' no HTML
...
    const luckyNumbersResult = document.getElementById('lucky-numbers-result');
    ...
    // <<< NOVO: Elementos para a seção de Escolha Aleatória
    ...
    const addRandomToCartBtn = document.getElementById('add-random-to-cart-btn'); // Novo botão para adicionar ao carrinho
    const randomNumbersMessage = document.getElementById('random-numbers-message'); // Para mensagens de erro/info
...
    let selectedNumbers = []; // Números selecionados manualmente pelo grid
    let randomSelectedNumbers = []; // <<< NOVO: Números selecionados aleatoriamente
    let rifaDocRef;
...
            raffleType = numbersData.type || 'dezena'; // Pega o tipo da rifa
            
...
            }
            renderNumberGrid(); // Chamada sem passar maxNumbers, pois o limite é interno agora
            if (loadingSection) loadingSection.classList.add('hidden');
            if (appSection) appSection.classList.remove('hidden');
            checkPaymentStatus();
            // <<< NOVO: Exibe ou esconde a seção de escolha aleatória conforme o tipo de rifa
...
    // <<< MODIFICADO: Adapta a renderização do grid de números para limitar a exibição
...
        numberGrid.innerHTML = '';
        ...
        // Mantive sua lógica original para as classes do grid.
...
        }
        ...
        // <<< NOVO: Lógica para limitar a exibição de números
        let numbersToRender = [];
        const availableNumbers = [];
        for (let i = 0; i < totalNumbersInRaffle; i++) {
            const numberStr = formatNumberForRaffleType(i, raffleType);
            if (!numbersData[numberStr]) { // Se o número não está comprado
                availableNumbers.push(numberStr);
            }
        }

        // Se a rifa for de dezena, exibe todos. Caso contrário, exibe até o limite.
        if (raffleType === 'dezena') {
            numbersToRender = availableNumbers; // Mostra todos os disponíveis
        } else {
            // Pega os primeiros `INITIAL_GRID_DISPLAY_LIMIT` números disponíveis
        ...
        if (numbersToRender.length === 0 && availableNumbers.length === 0) {
            ...
            if (!isRaffleOver) { // Se não terminou mas todos vendidos
            ...
            return;
             ...
             // Isso pode acontecer se, por exemplo, todos os primeiros 200 números estiverem vendidos
             // Neste caso, ainda exibiremos uma mensagem convidando a usar a seleção aleatória
             numberGrid.innerHTML = `<p class="text-center text-gray-500 col-span-full">Os primeiros números já foram comprados. Tente a opção "Escolha Aleatória" abaixo!</p>`;
             return;
        }


        numbersToRender.forEach(numberStr => { // Itera apenas sobre os números que serão renderizados
...
        });
        ...
        // Mensagem indicando que mais números estão disponíveis e incentivando a escolha aleatória
        if (availableNumbers.length > INITIAL_GRID_DISPLAY_LIMIT) {
            ...
            messageEl.textContent = `...e mais ${remainingCount} números disponíveis! Use a seção "Escolha Aleatória" abaixo para encontrar os seus!`;
...
    }
    ...
    // <<< NOVO: Função auxiliar para obter o comprimento esperado do número
...
        const expectedLength = getExpectedLengthForRaffleType(type); // Usa a nova função
...
        const raffleId = rifaDocRef.id;
        // <<< MODIFICADO: Considera números selecionados manualmente E aleatoriamente
...
                localStorage.setItem('pendingNumbers', JSON.stringify(numbersToBuy)); // Usa numbersToBuy
...
            
            const updates = {};
            // numbersToAttemptPurchase já vem formatado
...
            selectedNumbers = []; // Limpa os números selecionados manualmente
            randomSelectedNumbers = []; // <<< NOVO: Limpa os números aleatórios
            updateShoppingCart();
        } catch (e) {
...
    }
    
    // <<< NOVO: Função para gerar e exibir números aleatórios
    async function generateRandomNumbers() {
        ...
        addRandomToCartBtn.disabled = true; // Desabilita botão enquanto gera

        const availableNumbers = [];
        const expectedLength = getExpectedLengthForRaffleType(raffleType);

        // Coleta todos os números disponíveis
        for (let i = 0; i < totalNumbersInRaffle; i++) {
            const numberStr = formatNumberForRaffleType(i, raffleType);
            if (!numbersData[numberStr]) { // Se o número não está comprado
                availableNumbers.push(numberStr);
        ...
        if (quantity > availableNumbers.length) {
            ...
            let tempAvailable = [...availableNumbers]; // Cria uma cópia para embaralhar
            for (let i = 0; i < quantity; i++) {
                const randomIndex = Math.floor(Math.random() * tempAvailable.length);
                randomSelectedNumbers.push(tempAvailable[randomIndex]);
                tempAvailable.splice(randomIndex, 1); // Remove para não pegar o mesmo número
            }
            randomNumbersMessage.textContent = ''; // Limpa mensagem
        }
        
        // Exibe os números gerados na pequena tabela
        randomNumbersResultList.innerHTML = '';
    ...
    // <<< NOVO: Função para adicionar números aleatórios ao carrinho
    function addRandomNumbersToCart() {
        ...
        // Adiciona os números aleatórios à lista de selectedNumbers
        // E remove duplicatas, caso o usuário já tenha selecionado manualmente
        ...
        // Limpa a lista de números aleatórios exibidos
        randomSelectedNumbers = [];
        randomNumbersResultList.innerHTML = '';
        addRandomToCartBtn.classList.add('opacity-50', 'pointer-events-none');
        addRandomToCartBtn.disabled = true;
        randomNumbersMessage.textContent = ''; // Limpa mensagens

        // Re-renderiza o grid principal para refletir os números agora selecionados
        renderNumberGrid(); 
        updateShoppingCart(); // Atualiza o carrinho de compras
    }
...
    if (getLuckyNumbersBtn) getLuckyNumbersBtn.addEventListener('click', getLuckyNumbers);
    ...
    // <<< NOVO: Event listeners para a seção de Escolha Aleatória
