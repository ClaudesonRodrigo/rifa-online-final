...
// Esta função é chamada apenas quando o documento HTML está completamente carregado.
function initializeAdminApp() {
    ...
    onAuthStateChanged(auth, (user) => {
        ...
        // Verifica se o utilizador não é anónimo e tem um e-mail.
        if (user && !user.isAnonymous && user.email) {
            initializeAdminPanel(user);
        } else {
            // Se o utilizador for anónimo ou não existir, garante que ele é deslogado
            // e que a tela de login é mostrada.
            if(user) {
                signOut(auth);
            }
